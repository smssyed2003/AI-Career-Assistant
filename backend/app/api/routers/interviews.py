...
def list_interview_prep(job_id: int, candidate_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    return service.list_for_job(db, candidate_id=candidate_id, job_id=job_id, user_id=current_user.id)
