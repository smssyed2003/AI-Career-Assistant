...
def list_jobs(skip: int = 0, limit: int = 25, db: Session = Depends(get_db)):
...
async def ingest_job(
    ...
    job = service.create(db, job_in)
    return job

@router.get("/jobs/match/{candidate_id}", response_model=List[JobMatch], summary="Match candidate against job descriptions")
def match_candidate(candidate_id: int, top_k: int = 10, db: Session = Depends(get_db)):
    matches = matcher.match_candidate(db, candidate_id, top_k=top_k)
    return matches
...
def skill_gap(job_id: int, candidate_id: int, db: Session = Depends(get_db)):
    try:
        gap = matcher.analyze_skill_gap(db, candidate_id, job_id)
...
def interview_readiness(job_id: int, candidate_id: int, db: Session = Depends(get_db)):
    try:
        readiness = matcher.analyze_interview_readiness(db, candidate_id, job_id)
    except ValueError as exc:
        raise HTTPException(status_code=404, detail=str(exc))
    return readiness
